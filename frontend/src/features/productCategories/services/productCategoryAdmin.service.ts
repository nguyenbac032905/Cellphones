...
export const productCategoryAdminService = {
    getCategoryTree: async (): Promise<CategoryTreeResponse> => {
        const res = await privateAdmin.get<CategoryTreeResponse>("/admin/api/product-categories/tree");
        return res.data;
    },
    getAll: async (query: ProductCategoryQuery): Promise<CategoryListResponse> => {
        const res = await privateAdmin.get<CategoryListResponse>("/admin/api/product-categories", {params: query});
        return res.data;
    },
    create: async (body: CreateCategoryBody): Promise<MessageResponse> => {
        const res = await privateAdmin.post<MessageResponse>("/admin/api/product-categories", body);
        return res.data;
    },
    update: async (categoryID: string, body: UpdateCategoryBody): Promise<MessageResponse> => {
        const res = await privateAdmin.patch<MessageResponse>(`/admin/api/product-categories/${categoryID}`, body);
        return res.data;
    },
    get: async (categoryID: string): Promise<CategoryDetailResponse> => {
        const res = await privateAdmin.get<CategoryDetailResponse>(`/admin/api/product-categories/${categoryID}`);
        return res.data;
    },
    delete: async (categoryID: string): Promise<MessageResponse> => {
        const res = await privateAdmin.delete<MessageResponse>(`/admin/api/product-categories/${categoryID}`);
        return res.data;
    }
};